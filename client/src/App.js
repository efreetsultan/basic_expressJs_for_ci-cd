...
function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://EXT_IP:80/api/save-data`, {
        method: 'POST',
        headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`http://EXT_IP:80/api/delete-data`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`http://EXT_IP:80/api/data`);
      if (!response.ok) {
